  ...
  return (
    <details className="chart-palette chart-picker" onKeyDown={(event) => { if (event.key === 'Escape') { event.currentTarget.open = false; event.currentTarget.querySelector('summary')?.focus() } }}>
      ...
      <div className="chart-rgba">
        ...
        {opacity !== undefined && onOpacityChange && <label className="chart-alpha" title={opacityLabel}>T<input aria-label={`${label}: ${opacityLabel}`} type="number" min="0" max="100" step="1" value={Math.round((1 - opacity) * 100)} onChange={(event) => { if (event.target.value !== '') onOpacityChange(1 - Math.min(100, Math.max(0, Number(event.target.value))) / 100) }} /></label>}
      </div>
      </div>
    </details>
  )
